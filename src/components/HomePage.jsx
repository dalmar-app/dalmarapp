...
const HomePage = () => {
  ...
  const [deferredPrompt, setDeferredPrompt] = useState(null); // Kani waa Install Logic
  const navigate = useNavigate();

  // 1. Soo qaado sawirka (Database-kaaga)
  useEffect(() => {
    ...
    <div style={styles.container}>
      <style>
        {`
          ...
          .admin-overlay { position: absolute; bottom: 0; background: rgba(0, 0, 0, 0.7); color: #38bdf8; width: 100%; font-size: 11px; padding: 8px 0; text-align: center; }
        `}
      </style>
...
const styles = {
  ...
  installBtn: { marginTop: '15px', padding: '10px', width: '100%', backgroundColor: 'transparent', border: '1px solid #38bdf8', color: '#38bdf8', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold' },
